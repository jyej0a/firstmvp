...
- **기획 배경 & 문제 정의:**
  ...
  - **Workflow Fragmentation:** 소싱부터 Shopify 등록까지의 워크플로우가 수동 단계로 분산되어 있어, 각 단계마다 사용자 개입이 필요함. 대형 셀러는 자동화된 파이프라인으로 시장 진입 속도에서 우위를 점함.
  - **Automation Gap:** 트렌드 상품(미국) 데이터 수집, 원가 확인(중국), 가격 계산, Shopify API 연동까지의 전 과정이 수동 처리되어 Time-to-Market이 지연됨.
...
- **사용자 주요 니즈:** "틱톡에서 뜨는 거 봤어? 그거 아마존에 검색해서 데이터 확인하고 **클릭 한 번에** 내 Shopify 스토어에 자동 등록해줘."

## 2. 목적 및 핵심 가치 (KPI)

- **사용자 가치:**
  - **One-Click Automation:** 소싱부터 Shopify 등록까지 **클릭 한 번에 완전 자동화**. 수동 단계 제거로 Time-to-Market 최소화.
  - **Trend-Data Sync:** 틱톡(감성/바이럴)과 아마존(이성/구매) 데이터를 연결하여 **'팔릴 수밖에 없는 상품'**만 취급.
  - **Scalable Pipeline:** 안정적인 순차 처리(1분당 1개)로 하루 최대 1000개 상품 자동 등록 가능. Bot Detection 회피를 위한 안정적인 속도 유지.
  - **Global Sales Democratization:** 글로벌 세일즈의 기술적/운영적 장벽을 낮춰 1인 셀러도 해외 시장 진입 가능. 외화 획득 기회 확대 및 해외 수익 창출 지원.
- **핵심 지표 (KPI):**
  - **MVP 1.0 (2026-01-10)**
...
- **스크래핑 요소 (Phase 2.5):**
  - **필수 수집 항목 (Shopify 반영):**
    - 카테고리 (category): 아마존 카테고리 경로 추출 → Shopify `product_type`
    - 상세 설명 (description): HTML 또는 텍스트 형식 → Shopify `body_html`
    - 옵션 정보 (variants): 색상, 크기 등 옵션 배열 → Shopify `variants`
    - 브랜드명 (brand): 브랜드 정보 추출 → Shopify `vendor`
    - 무게 (weight): 킬로그램 단위 (파운드/온스 자동 변환) → Shopify `variants[].weight`
  - **부가 수집 항목 (DB/UI 표시용):**
    - 리뷰수 (review_count): 정수형
    - 평점 (rating): 0-5 범위 소수점
  - DB 스키마 확장: `category`, `review_count`, `rating`, `brand`, `weight` 컬럼 추가
  - Shopify 등록 시 필수 항목 자동 매핑
...
- **MVP 1.0:** 소싱부터 Shopify 등록까지 **클릭 한 번에 완전 자동화**로 Time-to-Market 최소화. 순차 처리 시스템(1분당 1개)으로 Bot Detection 위험을 최소화하면서 하루 최대 1000개 상품 자동 등록 가능. **글로벌 세일즈의 기술적/운영적 장벽을 낮춰 1인 셀러도 해외 시장 진입 가능하며, 외화 획득 기회 확대 및 해외 수익 창출 지원.**
- **v1.1:** 수집 개수 확장(50개) 및 CN 타입 지원으로 **소싱 옵션 다양화 및 마진 최적화**. 워크플로우 자동화 완성도 향상. 글로벌 세일즈 접근성 지속 개선.
